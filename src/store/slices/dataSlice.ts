...
const dataSlice = createSlice({
  ...
  reducers: {
    ...
    // Loading actions
    setLoading: (state, action: PayloadAction<{ key: keyof DataState['loading']; loading: boolean }>) => {
      const { key, loading } = action.payload;
      state.loading[key] = loading;
    },
    
    // Error actions
    setError: (state, action: PayloadAction<{ key: keyof DataState['errors']; error: string | null }>) => {
    ...
    updateCamera: (state, action: PayloadAction<Camera>) => {
      const index = state.cameras.findIndex(c => c._id === action.payload._id);
      if (index !== -1) {
        ...
        if (oldStatus !== newStatus && state.cameraStats) {
          if (oldStatus === 'Live' && newStatus === 'Offline') {
            state.cameraStats.data.activeCameras -= 1;
            state.cameraStats.data.offlineCameras += 1;
          } else if (oldStatus === 'Offline' && newStatus === 'Live') {
            state.cameraStats.data.activeCameras += 1;
            state.cameraStats.data.offlineCameras -= 1;
          }
        }
    ...
    updateAnomaly: (state, action: PayloadAction<Anomaly>) => {
      const index = state.anomalies.findIndex(a => a._id === action.payload._id);
      if (index !== -1) {
        ...
        const recentIndex = state.recentAnomalies.findIndex(a => a._id === action.payload._id);
        if (recentIndex !== -1) {
    ...
    removeAnomaly: (state, action: PayloadAction<string>) => {
      ...
      if (anomaly) {
        state.anomalies = state.anomalies.filter(a => a._id !== action.payload);
        state.recentAnomalies = state.recentAnomalies.filter(a => a._id !== action.payload);
