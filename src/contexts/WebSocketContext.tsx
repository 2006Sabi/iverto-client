...
export const WebSocketProvider: React.FC<WebSocketProviderProps> = ({ children }) => {
  ...
  const anomalyCallbacksRef = useRef<Set<(anomaly: Anomaly) => void>>(new Set());
  ...
  const initializeSocket = useCallback(() => {
    ...
    socket.on('connect', () => {
      ...
      toast({
        ...
        description: 'Live anomaly monitoring is now active',
        variant: 'default',
      });
    });
    ...
    socket.on('reconnect', (attemptNumber) => {
      ...
      toast({
        ...
        description: 'Real-time monitoring has been reconnected',
        variant: 'default',
      });
    });
    ...
    socket.on('reconnect_failed', () => {
      ...
      setIsConnected(false);
      dispatch(setReduxConnectionStatus('error'));
      dispatch(addConnectionError('Reconnection failed after maximum attempts'));
    ...
    socket.on('anomaly:new', (anomaly: any) => {
      ...
      if (normalized.camera_id && typeof normalized.camera_id === 'object') {
        normalized.camera_id = normalized.camera_id._id || normalized.camera_id.id;
      }
      if (normalized.camera && typeof normalized.camera === 'object' && normalized.camera._id) {
      ...
      toast({
        ...
        description: `${normalized.type} at ${normalized.location} (${normalized.confidence}% confidence)`,
        variant: 'destructive',
    ...
    socket.on('camera:status', (data: { id: string; status: 'Live' | 'Offline' }) => {
  ...
  const subscribeToAnomalies = useCallback((callback: (anomaly: Anomaly) => void) => {
