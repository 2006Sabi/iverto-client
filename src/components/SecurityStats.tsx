...
import { Camera, Shield, AlertTriangle, Eye, Clock, CheckCircle } from 'lucide-react';
import { useDashboardStats, useCameraStats, useAnomalyStats } from '@/hooks/useReduxData';
...
export const SecurityStats = () => {
  ...
  const mergedStats = useMemo(() => {
    ...
    const realtimeOfflineCount = Object.values(realtimeData.cameraStatusUpdates).filter(
      status => status === 'Offline'
    ).length;
    
    // Adjust counts based on real-time updates
    const adjustedActiveCameras = Math.max(0, baseActiveCameras - realtimeOfflineCount);
  ...
  }, [cameraStats?.data, dashboardStats?.data, anomalyStats?.data, realtimeData.cameraStatusUpdates]);
  ...
  const stats = [
    {
      ...
      change: mergedStats.offlineCameras > 0 ? `${mergedStats.offlineCameras} offline` : 'All online',
      icon: Camera,
    ...
    },
    {
      ...
      icon: AlertTriangle,
      status: mergedStats.highPriorityAnomalies > 0 ? 'alert' : 
             mergedStats.activeAnomalies > 0 ? 'warning' : 'success',
        ...
        <Card key={index} className="modern-card p-0 overflow-hidden">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-3 px-4 sm:px-6 pt-4 sm:pt-6">
            ...
            <div className={`p-2 sm:p-3 rounded-2xl ${
              stat.status === 'success' ? 'bg-green-500/10' :
              stat.status === 'warning' ? 'bg-yellow-500/10' :
              stat.status === 'alert' ? 'bg-red-500/10' : 'bg-primary/10'
            }`}>
              <stat.icon className={`h-4 w-4 sm:h-5 sm:w-5 ${
                stat.status === 'success' ? 'text-green-600' :
                stat.status === 'warning' ? 'text-yellow-600' :
                stat.status === 'alert' ? 'text-red-600' : 'text-primary'
              }`} />
            </div>
          </CardHeader>
            <CardContent className="px-4 sm:px-6 pb-4 sm:pb-6">
              <div className="flex items-center justify-between">
                <div className="min-w-0 flex-1">
                  <div className="text-2xl sm:text-3xl font-bold text-foreground mb-1 sm:mb-2 truncate">{stat.value}</div>
