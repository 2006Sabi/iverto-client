...
import { setUser, clearAuthData, setProfileChecked } from '@/store/slices/authSlice';
...
export const AuthGuard = ({ children }: AuthGuardProps) => {
  ...
  const { isAuthenticated, profileChecked, isLoading, user } = useAppSelector((state) => state.auth);
  ...
  }, [isAuthenticated, profileChecked, profileLoading, profileError, profileData, dispatch]);
  ...
  if (isAuthenticated && !profileChecked && (profileLoading || isLoading)) {
    return <PageLoader message="Verifying authentication..." />;
  }

  // If profile check failed, show loading while redirecting
  if (profileError) {
    return <PageLoader message="Authentication failed, redirecting..." />;
  }
